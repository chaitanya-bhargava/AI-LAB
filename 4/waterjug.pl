...
water_jug(X, Y) :- X<4, Y>3, write('3L water Jug 13 overflowed.'),nl.
...
water_jug(X, Y) :-  (X=:=0, Y=:=0, nl, write('4L:0 & 3L:3 (Action: Fill 3L jug.) '),YY is 3, water_jug(X,YY));
                    ...
                    (X=:=0, Y=:=3, nl, write('4L:3 & 3L:0 (Action: Pour water from 3L to 4L jug.) '),XX is X-3, YY is 0,  water_jug(XX,YY));
                    ...
                    (X=:=3, Y=:=3, nl, write('4L:4 & 3L:2 (Action: Pour water from 3L to 4L jug until 4L is full.) '),XX is X+1, YY is Y-1,water_jug(XX,YY));
                    ...
                    (X=:=4, Y=:=1, nl, write('4L:2 & 3L:3 (Action: Pour water from 4L to 3L jug until 3L is full .) '),XX is X-2, YY is Y+2,water_jug(XX,YY));
                    ...
                    (X=:=0, Y=:=2, nl, write('4L:2 & 3L:0 (Action: Pour water from 3L to 4L jug) '),XX is Y,YY is X,  water_jug(XX,YY)).
